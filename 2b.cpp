...
int main(int argc, char *argv[])
{
    ...
    for (auto x : input)
    {
        ...
        auto low_int = std::stoi(low) - 1;
        auto high_int = std::stoi(high) - 1;
        if ((value.at(low_int) == letter_char) && value.at(high_int) != letter_char)
        {
            result += 1;
        }
        else if ((value.at(low_int) != letter_char) && value.at(high_int) == letter_char)
        {
            result += 1;
        }
    }
